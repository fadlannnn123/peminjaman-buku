...
@section('subtitle', 'Ubah data katalog buku perpustakaan')
                ...
                <div>
                    ...
                    <select id="kategori_id" name="kategori_id" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                        ...
                        @foreach($kategoris as $kat)
                            <option value="{{ $kat->id_kategori }}" {{ old('kategori_id', $book->kategori_id) == $kat->id_kategori ? 'selected' : '' }}>
                                {{ $kat->nama_kategori }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    @if($book->foto)
                        <div class="mb-4">
                            ...
                            <div class="w-24 h-24 rounded-lg border border-gray-200 overflow-hidden shadow-sm">
                                <img src="{{ \Illuminate\Support\Facades\Storage::url($book->foto) }}" alt="FotoBuku" class="w-full h-full object-cover">
                            </div>
                        </div>
                    @endif
                        ...
                        <div class="space-y-1 text-center">
                            ...
                            <div class="flex text-sm text-gray-600 justify-center mt-2">
                                <label for="foto" class="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500 px-2 py-1 shadow-sm border border-indigo-100">
                                    ...
                                    <input id="foto" name="foto" type="file" class="sr-only" accept="image/*" onchange="document.getElementById('file-name-edit').textContent = this.files[0] ? this.files[0].name : 'Tetap gunakan foto lama'; document.getElementById('file-name-edit').classList.add('text-indigo-600', 'font-bold');">
                                </label>
                            </div>
                            <p id="file-name-edit" class="text-xs text-gray-500 mt-2">Tetap gunakan foto lama bila kosong</p>
                        </div>
                    </div>
